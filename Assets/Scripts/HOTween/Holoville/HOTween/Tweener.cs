...
public class Tweener : ABSTweenComponent
{
	...
	{
		Rewind(p_skipDelay: false);
	}

	public void Rewind(bool p_skipDelay)
	{
		Rewind(p_play: false, p_skipDelay);
	}

	public override void Restart()
	{
		Restart(p_skipDelay: false);
	}
	...
	public void Restart(bool p_skipDelay)
	{
		...
		{
			Rewind(p_play: true, p_skipDelay);
		}
	}
	...
	public Tweener UsePartialPath(int p_waypointId0, int p_waypointId1, float p_newDuration, EaseType p_newEaseType)
	{
		...
		int num = ConvertWaypointIdToPathId(pv3Path, p_waypointId0, p_isStartingWp: true);
		int p_pathWaypointId = ConvertWaypointIdToPathId(pv3Path, p_waypointId1, p_isStartingWp: false);
		...
		Startup(p_force: true);
		if (!_isPaused)
		{
			Restart(p_skipDelay: true);
		}
		else
		{
			Rewind(p_skipDelay: true);
		}
		return this;
	}
	...
	private PlugVector3Path GetPlugVector3PathPlugin()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (aBSTweenPlugin is PlugVector3Path result)
			{
				return result;
			}
		}
	...
	internal bool Update(float p_shortElapsed, bool p_forceUpdate, bool p_isStartupIteration, bool p_ignoreCallbacks, bool p_ignoreDelay = false)
	{
		...
		{
			Kill(p_autoRemoveFromHOTween: false);
			return true;
		}
		if (!_enabled)
		{
	...
	protected override bool GoTo(float p_time, bool p_play, bool p_forceUpdate, bool p_ignoreCallbacks)
	{
		...
		Update(0f, p_forceUpdate: true, p_isStartupIteration: false, p_ignoreCallbacks);
