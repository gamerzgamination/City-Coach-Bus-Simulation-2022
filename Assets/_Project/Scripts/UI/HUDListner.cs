...
public class HUDListner : MonoBehaviour
{
    ...
    public Image ParkingBar;
    public GameObject WarningAlertMessage;
    ...
    public GameObject InteriorDasboardBus;
    [Header("sprites")]
    ...
    public void set_statusEngine(bool _val)
    {
        if (_val)
            EngineButton.GetComponent<Image>().sprite = engineOnOff[0];
        else
            EngineButton.GetComponent<Image>().sprite = engineOnOff[1];

    }
    public void set_statusInteriorbus(bool _val)
    {
        InteriorDasboardBus.SetActive(_val);
    }
    public void set_StatusParkingbar(bool _val)
    {
        ParkingBar.gameObject.SetActive(_val);
    }
    public void set_StatusFuelBar(float _val,bool refuel)
    {
        if (!refuel)
        {
            if (Toolbox.GameplayController.SelectedvehicleRCCV3.speed > 7)
                Fuelbar.fillAmount -= _val * Time.deltaTime;

            if (Fuelbar.fillAmount > 0.5f)
                Fuelbar.color = Color.green;

            else if (Fuelbar.fillAmount < 0.5f && Fuelbar.fillAmount > 0.3f)
                Fuelbar.color = Color.yellow;

            else if (Fuelbar.fillAmount < 0.3f)
