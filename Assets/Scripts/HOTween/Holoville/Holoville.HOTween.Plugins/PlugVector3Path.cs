...
public class PlugVector3Path : ABSTweenPlugin
{
	...
	{
		return OrientToPath(p_orient: true);
	}
	...
	{
		return OrientToPath(p_orient: true, p_lookAhead, Axis.None);
	}

	public PlugVector3Path OrientToPath(Axis p_lockRotationAxis)
	{
		return OrientToPath(p_orient: true, 0.0001f, p_lockRotationAxis);
	}

	public PlugVector3Path OrientToPath(float p_lookAhead, Axis p_lockRotationAxis)
	{
		return OrientToPath(p_orient: true, p_lookAhead, p_lockRotationAxis);
	}
	...
	protected override void SetChangeVal()
	{
		...
		if (isRelative)
		{
			...
			{
				ref Vector3 reference = ref array[i + num];
		...
		else
		{
			...
			for (int i = 0; i < num3; i++)
			{
				ref Vector3 reference2 = ref array[i + num];
				reference2 = points[i];
			}
		}
		num3 = array.Length;
		if (isClosedPath)
		{
			ref Vector3 reference3 = ref array[num3 - 2];
			reference3 = array[1];
		}
		if (isClosedPath)
		{
			ref Vector3 reference4 = ref array[0];
			reference4 = array[num3 - 3];
			ref Vector3 reference5 = ref array[num3 - 1];
			reference5 = array[2];
		}
		else
		{
			ref Vector3 reference6 = ref array[0];
			reference6 = array[1];
			Vector3 vector3 = array[num3 - 2];
			Vector3 vector4 = vector3 - array[num3 - 3];
			ref Vector3 reference7 = ref array[num3 - 1];
			reference7 = vector3 + vector4;
		}
		if (lockPositionAxis != 0)
		{
			...
			for (int i = 0; i < num3; i++)
			{
				...
				ref Vector3 reference8 = ref array[i];
				reference8 = new Vector3(flag ? vector5.x : vector6.x, flag2 ? vector5.y : vector6.y, flag3 ? vector5.z : vector6.z);
			}
		}
	...
	protected override void DoUpdate(float p_totElapsed)
	{
		...
		Vector3 constPointOnPath = GetConstPointOnPath(pathPerc, p_updatePathPerc: true, path, out out_waypointIndex);
	...
	{
		int out_waypointIndex;
		return GetConstPointOnPath(t, p_updatePathPerc: false, null, out out_waypointIndex);
	}
