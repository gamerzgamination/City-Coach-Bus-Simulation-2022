...
public class TweenParms : ABSTweenComponentParms
{
	...
	internal void InitializeObject(Tweener p_tweenObj, object p_target)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (hOTPropData.endValOrPlugin is ABSTweenPlugin aBSTweenPlugin)
			{
	...
	{
		return SpeedBased(p_speedBased: true);
	}
	...
	{
		return NewProp(p_propName, p_plugin, p_isRelative: false);
	}

	public TweenParms NewProp(string p_propName, object p_endVal)
	{
		return NewProp(p_propName, p_endVal, p_isRelative: false);
	}
	...
	{
		return Prop(p_propName, p_plugin, p_isRelative: false);
	}

	public TweenParms Prop(string p_propName, object p_endVal)
	{
		return Prop(p_propName, p_endVal, p_isRelative: false);
	}
