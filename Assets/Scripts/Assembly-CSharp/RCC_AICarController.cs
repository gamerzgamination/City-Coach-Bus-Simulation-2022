...
public class RCC_AICarController : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		navigatorObject.GetComponent<NavMeshAgent>().radius = 1f;
		navigatorObject.GetComponent<NavMeshAgent>().speed = 1f;
		navigatorObject.GetComponent<NavMeshAgent>().height = 1f;
	...
	private void Navigation()
	{
		...
		if (!inBrakeZone)
		{
			if (carController.speed >= 10f)
			{
				...
				{
					gasInput = Mathf.Clamp(1f - (Mathf.Abs(num / 5f) - Mathf.Abs(rayInput / 5f)), 0.5f, 1f);
				}
				else
				{
					gasInput = 0f;
	...
	private void Resetting()
	{
		...
		if (resetTime >= 6f || carController.speed >= 25f)
		{
			carController.direction = 1;
			resetTime = 0f;
		}
