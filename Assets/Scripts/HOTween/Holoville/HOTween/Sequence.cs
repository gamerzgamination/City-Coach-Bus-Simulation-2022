...
public class Sequence : ABSTweenComponent
{
	...
	internal override void Kill(bool p_autoRemoveFromHOTween)
	{
		...
		if (items != null)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				{
					hOTSeqItem.twMember.Kill(p_autoRemoveFromHOTween: false);
				}
			}
			items = null;
		}
	...
	public List<Tweener> GetTweenersByTarget(object p_target)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (hOTSeqItem.twMember is Tweener tweener)
			{
		...
		if (_duration > 0f)
		{
			...
			for (int i = count - 1; i > -1; i--)
			{
				...
				if (hOTSeqItem.seqItemType == SeqItemType.Tween && hOTSeqItem.startTime > num5)
				{
					...
					{
						hOTSeqItem.twMember.GoTo(num5 - hOTSeqItem.startTime, p_forceUpdate, p_ignoreCallbacks: true);
					}
					else
					{
	...
	private void TweenStartupIteration()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (hOTSeqItem.seqItemType == SeqItemType.Tween)
			{
				hOTSeqItem.twMember.Update(hOTSeqItem.twMember.duration, p_forceUpdate: true, p_isStartupIteration: true);
			}
		}
		for (int i = count - 1; i > -1; i--)
		{
	...
	{
		if (p_twMember is Tweener tweener && tweener._speedBased)
		{
	...
	internal override void FillPluginsList(List<ABSTweenPlugin> p_plugs)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (hOTSeqItem.twMember != null)
			{
				if (hOTSeqItem.twMember is Sequence sequence)
				{
